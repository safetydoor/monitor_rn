...
- (void)viewForCell:(NSUInteger)cell inScroll:(NSUInteger)scroll reusingCell:(UITableViewCell *)tc {
    ...
    float value = fTimeZoneVal[cell];
    switch (scroll) {
        case 0:{
            if(value<0){
                tc.textLabel.text=[NSString stringWithFormat:@"UTC %g",value];
            }else{
                tc.textLabel.text=[NSString stringWithFormat:@"UTC %g",value];
            }
...
- (void)didSelectCell:(NSUInteger)cell inScroll:(NSUInteger)scroll {
    ...
    if(scroll==0){
        if(self.delegate!=nil){
            [self.delegate onTimezoneChange:fTimeZoneFlag[cell]];
        }
        _timezone = fTimeZoneFlag[cell];
    }
}

//选择当前cell
-(void)selectedCellFromTimezoneView:(NSUInteger)flag{
    NSUInteger cell = timeZoneFlag[flag];
    [_picker selectCell:cell inScroll:0];
}
