...
- (void)selectCell:(NSUInteger)cell inScroll:(NSUInteger)scroll {
    ...
    if (dataLoop) {
        [sc setContentOffset:CGPointMake(sc.contentOffset.x, (cell+1-(_selectionPosition+1)+[[_numberOfCellsInScroll objectAtIndex:scroll]intValue])*rowHeight) animated:NO];
    } else {
        [sc setContentOffset:CGPointMake(sc.contentOffset.x, cell*rowHeight) animated:NO];
    }
}
